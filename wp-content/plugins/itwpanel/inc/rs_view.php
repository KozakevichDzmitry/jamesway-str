...
?>
<style>
@import url(https://fonts.googleapis.com/css?family=Roboto+Condensed:400,300,700&subset=latin,latin-ext,cyrillic,cyrillic-ext);
/*Login page*/
...
body.login h1 a{
	<?php if($option['logo']) { ?> background: url(<?php echo $option['logo']; ?>) no-repeat center!important; 
	<?php } else { ?> background: url(<?php echo RS_DIR; ?>assets/img/itw-logo.svg) no-repeat center!important;<?php } ?>
	width: 100% !important;
	height: 40px !important;
}
body.wp-core-ui .button-primary, #submit{
	position: relative;
	display: inline-block;
	background: <?php if($color) { echo $color; }  else { echo '#81a93a;'; } ?>;
	text-shadow: none;
	padding: 0px 25px;
	height: 45px;
	line-height: 45px;
...
.wp-core-ui .button-primary:hover, #submit:hover{
	background: #000;
}
...
body.login #nav a:focus, body.login #backtoblog a:focus, body.login #nav a:hover, body.login #backtoblog a:hover{
	color: <?php if($color) { echo $color; }  else { echo '#81a93a;'; } ?>;
}

#loginform input:focus{
	border-color: <?php if($color) { echo $color; }  else { echo '#81a93a;'; } ?>;
	box-shadow: none;
}
#loginform .input{
	color: #8e8e8e;
